...
<a   class="modal-trigger waves-effect  waves-light btn gradient-45deg-light-blue-cyan z-depth-3  mr-3 btn-small" href="{{route('admin.sub-category.image-edit', [$record->id] )}}">
        Edit
    </a>
 ...
 <a class="modal-trigger waves-effect  waves-light btn gradient-45deg-light-blue-cyan z-depth-3  mr-3 btn-small"
                    ...
                    href= "{{route('admin.sub-category.edit',$record->id  )}}">Edit</a>
       <form class="display-inline" method="POST" action={{route('admin.sub-category.destroy', [$record->id]  )}}>
           @csrf
           @method('delete')
